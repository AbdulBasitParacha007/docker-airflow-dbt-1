...
: "${AIRFLOW__CORE__DAGS_FOLDER:="/usr/local/airflow"}"
: "${AIRFLOW__CORE__FERNET_KEY:=${FERNET_KEY:=$(python -c "from cryptography.fernet import Fernet; FERNET_KEY = Fernet.generate_key().decode(); print(FERNET_KEY)")}}"
: "${AIRFLOW__CORE__EXECUTOR:=${EXECUTOR:-Local}Executor}"
: "${_AIRFLOW_WWW_USER_CREATE:= "true"}"
: "${_AIRFLOW_WWW_USER_USERNAME:= "admin"}"
: "${_AIRFLOW_WWW_USER_PASSWORD:= "admin"}"
: "${DIR_PROJECT_DBT:="/usr/local/airflow/dbt"}"
: "${DIR_PROFILE_DBT:="/usr/local/airflow/dbt"}"
: "${DMX_META_USERNAME:="boltwire_administrator"}"
: "${DMX_META_PASSWORD:="12345"}"
: "${DMX_META_HOST:="10.51.100.64"}"
: "${DMX_META_PORT:="5432"}"
: "${DMX_META_DATABASE:="dmx_dev"}"
: "${DMX_META_JDBC_URL:="jdbc:postgresql://10.51.100.64:5432/dmx_dev"}"
: "${DMX_META_DRIVER:="org.postgresql.Driver"}"
